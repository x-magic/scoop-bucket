...
version_check_url = 'https://dotnet.microsoft.com/download/dotnet/8.0'
manifest_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), '..', 'bucket', 'windowsdesktop-runtime-lts-8.0.x-{arch}.json')
archs = ['x86', 'x64']

print("=====> windowsdesktop-runtime-lts-8.0.x")
    ...
    with urllib.request.urlopen(version_check_url) as url:
        ...
        if version_current != version_existing:
            ...
            with urllib.request.urlopen(download_page_url) as download_page:
                ...
                message = "windowsdesktop-runtime-lts-8.0.x-{0}: Update to version {1}".format(arch, version_current)
