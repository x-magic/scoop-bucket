...
version_check_url = 'https://www.diskgenius.cn/download.php'
...
try:
    with urllib.request.urlopen(version_check_url) as url:
        version_current = re.findall('V(\\d+.\\d+.\\d+.\\d+)', url.read().decode())[0]
        version_existing = manifest['version']

        if version_current != version_existing:
            print("New version {0} detected (current version {1}). Will update the manifest...".format(
                version_current,
                version_existing)
            )
            archs = ['32bit', '64bit']
            ...
            for arch in archs:
                download_url = manifest['autoupdate']['architecture'][arch]['url'].replace('$cleanVersion',
                                                                                           version_current.replace('.',
                                                                                                                   ''))
